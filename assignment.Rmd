...
pie_data_top =
  pie_data %>%
  group_by(user.id) %>%
  filter(hours_perc > 0.05) %>%
  arrange(desc(hours_perc)) %>%
  slice(1:4) %>% # Keep only top k played for each gamer
  ungroup()
...
to_plot = add_row(
  ...
  pie_data_top %>% add_row(pie_data_others) %>%
    ...
    mutate(label_type = "hours_perc") %>%
    
    mutate(lab.ypos = cumsum(hours_perc) - hours_perc/2) %>%
    #mutate(lab.xpos = 0.3 + (1-hours_perc*.6)) %>%
    #mutate(lab.xpos = 1.7) %>%
    mutate(lab.xpos = 1.5) %>%
    ungroup(),
  
  pie_data_top %>% add_row(pie_data_others) %>%
    mutate(label=paste(hours,"h",sep="")) %>%
    arrange(user.id, desc(game.title)) %>%
    group_by(user.id) %>%
    
    mutate(label_type = "hours") %>%
  ...
  pie_data_top %>% add_row(pie_data_others) %>%
    mutate(label=game.title) %>%
    arrange(user.id, desc(game.title)) %>%
    group_by(user.id) %>%
    
    mutate(label_type = "game.title") %>%
...
```{r}
to_plot %>%
  filter(label_type == "game.title") %>% # TODO remove
  ...
  coord_polar("y") +
  #geom_text(aes(y = lab.ypos, label = label), color = "black") + #, size=6) + 
  #geom_text(aes(x = lab.xpos, y = lab.ypos, label = label), color = "black") +
  
  #geom_label_repel(
  geom_label(
    aes(label = label, x=lab.xpos, y=lab.ypos),
    #box.padding   = 0.3, 
    #point.padding = 0.0,
    #segment.color = 'grey50',
    #label.padding = 0.2,
  ...
  #scale_fill_manual(values= brewer.pal(n=nrow(to_plot %>% distinct(game.title)), name = "Pastel1")) +
