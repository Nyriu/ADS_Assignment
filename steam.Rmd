...
Usiamo un altro CSV per capire quali sono veramente giochi

```{r}
raw_data_2 = as_tibble(
  read.csv("steam_games.csv"
           #, header=F,
           #col.names = c( "user-id","game-title", "behavior-name", "value", "unknown")
           )
  ) %>% select(-url, -recent_reviews, -all_reviews, -mature_content, -minimum_requirements, -recommended_requirements, -discount_price)
head(raw_data_2)
colnames(raw_data_2)
```
